...
final class ThemeManager {
    ...
    ///
    /// 仅保存用户设置值；全屏覆盖（强制 1.0）由 ContentView 按 per-window
    /// `isFullScreen` 在视图层完成，避免全局单例污染多窗口。
    ...
    ///
    /// 仅保存用户设置值；全屏覆盖由 ContentView 按 per-window `isFullScreen`
    /// 在视图层完成。
    private(set) var contentOpacity: CGFloat = 1.0

    /// 中间内容层用户设置的不透明度 = backgroundOpacity × contentOpacity。所有
    /// 画"中间内容"底色的视图都读它，以保证两档设置的组合效果一致。
    /// 全屏覆盖不在此计算 —— ContentView 在 per-window 注入
    /// `\.effectiveContentOpacity` 时叠加全屏因子。
